...
export function extractJsonLdProducts(html: string, marketplace: Marketplace, sourceUrl: string): ParsedProduct[] {
  ...
  for (const match of scripts) {
    try {
      ...
      const nodes: unknown[] = Array.isArray(parsed) ? parsed : (parsed && typeof parsed === "object" && Array.isArray((parsed as Record<string, unknown>)["@graph"]) ? (parsed as Record<string, unknown>)["@graph"] as unknown[] : [parsed]);
