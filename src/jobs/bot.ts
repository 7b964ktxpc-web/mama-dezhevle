...
async function main() {
  ...
  try {
    ...
    for (const update of updates) {
      ...
      const callback = update.callback_query; if (callback) { await answerTelegramCallback(callback.id); const chatId = callback.message?.chat.id; if (!chatId) continue; if (callback.data === "menu:search") await sendTelegramBotMessage(chatId, "🔎 Просто напиши, что ищем — я продолжу разговор сама.", mainMenuKeyboard()); else if (callback.data === "menu:photo") await sendTelegramBotMessage(chatId, "📸 Пришли фотографию товара — попробую определить его и продолжу поиск.", mainMenuKeyboard()); else if (callback.data === "menu:watches") await sendTelegramBotMessage(chatId, "🔔 Чтобы посмотреть подписки, используй /watches.", mainMenuKeyboard()); else if (callback.data === "menu:support" || callback.data === "support:start") await sendTelegramBotMessage(chatId, "💬 Я здесь 🙂 Просто напиши мне сообщение обычным текстом.", mainMenuKeyboard()); else if (callback.data?.startsWith("watch:")) await sendTelegramBotMessage(chatId, `🔔 Чтобы следить за товаром, отправь /watch ${callback.data.slice(6)}.`, mainMenuKeyboard()); continue; }
      ...
      if (text === "/watches") { await sendTelegramBotMessage(message.chat.id, "🔔 Раздел подписок подключён. Можешь продолжить обычный диалог со мной.", mainMenuKeyboard()); continue; }
