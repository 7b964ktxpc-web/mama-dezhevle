  ...
  try {
    ...
    for (const update of updates) {
      ...
      if (text === "/watches") {
        ...
        if (error) throw error;
        const watches = (data ?? []) as Array<{ product_id: string; target_price: number | null; products: Array<{ title: string }> | null }>;
        await sendTelegramBotMessage(message.chat.id, watches.length ? ["🔔 Мои подписки:", "", ...watches.map((item) => `• ${item.products?.[0]?.title ?? item.product_id}${item.target_price ? ` — до ${Math.round(Number(item.target_price))} ₽` : " — любое снижение"}`)].join("\n") : "🔕 Активных подписок нет.");
        continue;
      }
