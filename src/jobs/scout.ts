...
import { gatewaySearch, type Offer } from "../lib/kettu-gateway";

// Background Scout: periodically sweeps popular kids-product queries across
// marketplaces, records prices into products/prices and files high-scoring
// offers as candidate deals for the admin panel.
...
async function sweepOnce() {
  ...
  const queries = (process.env.SCOUT_QUERIES?.trim() ? process.env.SCOUT_QUERIES.split(",").map((q) => q.trim()).filter(Boolean) : DEFAULT_QUERIES);
  let inserted = 0;
...
async function runLoop() {
  const intervalMinutes = Number(process.env.SCOUT_INTERVAL_MINUTES) || 60;
  while (true) {
    ...
    console.log(JSON.stringify({ event: "scout_sleep", minutes: intervalMinutes }));
